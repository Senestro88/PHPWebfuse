...
if (!class_exists("\ArchiveZip")) {
	...
	class ArchiveZip {
		...
		public function create($filelist, $params = 0) {
			...
			if ($this->_check_parameters($params,
		...
		public function duplicate($archive) {
			...
			if ((is_object($archive))
				&& (strtolower(get_class($archive)) == 'archive_zip')) {
				$v_result = $this->_duplicate($archive->_zipname);
			} else if (is_string($archive)) {
		...
		public function merge($archive_to_add) {
			...
			if ((is_object($archive_to_add))
				&& (strtolower(get_class($archive_to_add)) == 'archive_zip')) {
				$v_result = $this->_merge($archive_to_add);
			} else if (is_string($archive_to_add)) {
		...
		public function _add($list, &$result_list, &$params) {
			...
			// Add the files
			$v_header_list = array();
			if (($v_result = $this->_addFileList($list, $v_header_list,
				$add_dir, $remove_dir,
				$remove_all_dir, $params)) != 1) {
		 ...
		 * @return int
		 */
		public function _addList($list, &$result_list,
			$add_dir, $remove_dir, $remove_all_dir, &$params) {
			$v_result = 1;
		 ...
		 * @return int
		 */
		public function _addFileList($list, &$result_list,
			$add_dir, $remove_dir, $remove_all_dir,
			&$params) {
			$v_result = 1;
			$v_header = array();
		...
		public function _addFile($filename, &$header, $add_dir, $remove_dir, $remove_all_dir, &$params) {
			...
			*/
			...
			if ((isset($params[ARCHIVE_ZIP_PARAM_PRE_ADD]))
				&& ($params[ARCHIVE_ZIP_PARAM_PRE_ADD] != '')) {
			...
			// Look for pre-add callback
			if ((isset($params[ARCHIVE_ZIP_PARAM_POST_ADD]))
				&& ($params[ARCHIVE_ZIP_PARAM_POST_ADD] != '')) {
		...
		public function _writeFileHeader(&$header) {
			...
			// Packed data
			$v_binary_data = pack("VvvvvvVVVvv", 0x04034b50, $header['version'], $header['flag'],
				$header['compression'], $v_mtime, $v_mdate,
				$header['crc'], $header['compressed_size'], $header['size'],
				strlen($header['stored_filename']), $header['extra_len']);
		...
		public function _writeCentralFileHeader(&$header) {
			...
			// Packed data
			$v_binary_data = pack("VvvvvvvVVVvvvvvVV", 0x02014b50, $header['version'], $header['version_extracted'],
				$header['flag'], $header['compression'], $v_mtime, $v_mdate, $header['crc'],
				$header['compressed_size'], $header['size'],
				strlen($header['stored_filename']), $header['extra_len'], $header['comment_len'],
				$header['disk'], $header['internal'], $header['external'], $header['offset']);
		...
		public function _extractByRule(&$file_list, &$params) {
			...
			if (($path == "")
				|| ((substr($path, 0, 1) != "/")
					&& (substr($path, 0, 3) != "../") && (substr($path, 1, 2) != ":/"))) {
				$path = "./" . $path;
			}
			...
			for ($i = 0, $v_nb_extracted = 0; $i < $v_central_dir['entries']; $i++) {
				...
				if ((isset($params[ARCHIVE_ZIP_PARAM_BY_NAME]))
					&& ($params[ARCHIVE_ZIP_PARAM_BY_NAME] != 0)) {
					...
					for ($j = 0; ($j < sizeof($params[ARCHIVE_ZIP_PARAM_BY_NAME])) && (!$v_extract); $j++) {
						...
						if (substr($params[ARCHIVE_ZIP_PARAM_BY_NAME][$j], -1) == "/") {
							...
							if ((strlen($v_header['stored_filename']) > strlen($params[ARCHIVE_ZIP_PARAM_BY_NAME][$j]))
								&& (substr($v_header['stored_filename'], 0, strlen($params[ARCHIVE_ZIP_PARAM_BY_NAME][$j])) == $params[ARCHIVE_ZIP_PARAM_BY_NAME][$j])) {
								$v_extract = true;
							}
				...
				} else if ((isset($params[ARCHIVE_ZIP_PARAM_BY_PREG]))
					&& ($params[ARCHIVE_ZIP_PARAM_BY_PREG] != "")) {
					// Look for extract by preg rule
					if (preg_match($params[ARCHIVE_ZIP_PARAM_BY_PREG], $v_header['stored_filename'])) {
						$v_extract = true;
					}
				} else if ((isset($params[ARCHIVE_ZIP_PARAM_BY_INDEX]))
					&& ($params[ARCHIVE_ZIP_PARAM_BY_INDEX] != 0)) {
		...
		public function _extractFile(&$entry, $path, $remove_path, $remove_all_path, &$params) {
			...
			if ((isset($params[ARCHIVE_ZIP_PARAM_PRE_EXTRACT]))
				&& ($params[ARCHIVE_ZIP_PARAM_PRE_EXTRACT] != '')) {
			...
			if ($entry['status'] == 'ok') {
				...
				if (file_exists($entry['filename'])) {
					...
					if (is_dir($entry['filename'])) {
						...
						//return $v_result;
					} else if (!is_writeable($entry['filename'])) {
						// Look if file is write protected
						// Change the file status
						$entry['status'] = "write_protected";

						//return $v_result;
					} else if (filemtime($entry['filename']) > $entry['mtime']) {
			...
			if ($entry['status'] == 'ok') {
				...
				if (!(($entry['external']&0x00000010) == 0x00000010)) {
					...
					if ((isset($params[ARCHIVE_ZIP_PARAM_SET_CHMOD]))
						&& ($params[ARCHIVE_ZIP_PARAM_SET_CHMOD] != 0)) {
			...
			if ((isset($params[ARCHIVE_ZIP_PARAM_POST_EXTRACT]))
				&& ($params[ARCHIVE_ZIP_PARAM_POST_EXTRACT] != '')) {
		...
		public function _deleteByRule(&$result_list, &$params) {
				...
				if ((isset($params[ARCHIVE_ZIP_PARAM_BY_NAME]))
					...
					for ($j = 0; ($j < sizeof($params[ARCHIVE_ZIP_PARAM_BY_NAME])) && (!$v_found); $j++) {
						...
						if (substr($params[ARCHIVE_ZIP_PARAM_BY_NAME][$j], -1) == "/") {
							...
							// Look if the directory is in the filename path
							if ((strlen($v_header_list[$v_nb_extracted]['stored_filename']) > strlen($params[ARCHIVE_ZIP_PARAM_BY_NAME][$j]))
								&& (substr($v_header_list[$v_nb_extracted]['stored_filename'], 0, strlen($params[ARCHIVE_ZIP_PARAM_BY_NAME][$j])) == $params[ARCHIVE_ZIP_PARAM_BY_NAME][$j])) {
								$v_found = true;
							} elseif ((($v_header_list[$v_nb_extracted]['external']&0x00000010) == 0x00000010) /* Indicates a folder */
								&& ($v_header_list[$v_nb_extracted]['stored_filename'] . '/' == $params[ARCHIVE_ZIP_PARAM_BY_NAME][$j])) {
								$v_found = true;
							}
					...
					&& ($params[ARCHIVE_ZIP_PARAM_BY_PREG] != "")) {
					// Look for extract by preg rule
					if (preg_match($params[ARCHIVE_ZIP_PARAM_BY_PREG],
					...
					for ($j = $j_start; ($j < sizeof($params[ARCHIVE_ZIP_PARAM_BY_INDEX])) && (!$v_found); $j++) {
						...
						if (($i >= $params[ARCHIVE_ZIP_PARAM_BY_INDEX][$j]['start'])
							&& ($i <= $params[ARCHIVE_ZIP_PARAM_BY_INDEX][$j]['end'])) {
							$v_found = true;
						}
						if ($i >= $params[ARCHIVE_ZIP_PARAM_BY_INDEX][$j]['end']) {
							$j_start = $j + 1;
			...
			if ($v_nb_extracted > 0) {
				...
				for ($i = 0; $i < sizeof($v_header_list); $i++) {
					...
					$v_result = $this->_tool_CopyBlock($this->zip_fd,
						$v_temp_zip->_zip_fd,
		...
		public function _tool_CopyBlock($src, $dest, $size, $mode = 0) {
			...
			if ($mode == 0) {
				while ($size != 0) {
					$v_read_size = ($size < ARCHIVE_ZIP_READ_BLOCK_SIZE
						? $size : ARCHIVE_ZIP_READ_BLOCK_SIZE);
