...
class Time {
	...
	public function getDate(?int $timestamp = null): array {return getdate($timestamp);}
	...
	public static function FormatTimeAgoVersion1(int $timestamp): string {
		...
		if ($difference < 5) {return 'less than 5 seconds ago';}
	...
	public static function FormatTimeAgoVersion2(int $timestamp): string {
		...
		foreach ($since as $unit => $value) {
			if ($string) {$string .= $unit !== $lastValue ? ', ' : ' and ';}
