 ...
 *
 */
class Http extends \PHPWebFuse\Methods {
	// PRIVATE CONSTANTS

	private const methods = ['GET', 'POST', 'HEAD'];
	...
	public function request(string $method, string $url, ?array $headers = null, array $params = []): mixed {
		...
		if (parent::isNotInArray($method, self::methods)) {$this->message = "The request method must be one of the following: " . implode(", ", self::methods);} else if (!function_exists('curl_init')) {$this->message = "Curl extension isn't loaded";} else {
			$curl = curl_init();
			if (parent::isFalse($curl)) {$this->message = "Can't initialize Curl handle";} else {
				$_headers = [];
				if (self::isArray($headers)) {foreach ($headers as $key => $value) {$_headers[] = $key . ": " . $value;}}
				if (isset($_headers['Content-Type']) && $method == "POST") {$_headers['Content-Type'] = "multipart/form-data";}
				$options = [
					...
					CURLOPT_USERAGENT => parent::userAgent,
				...
				if (parent::isFalse(curl_setopt_array($curl, $options))) {$this->message = "Failed to set Curl options";}
				else{
					$response = curl_exec($curl);
					if (parent::isFalse($response)) {$this->message = "Failed to execute Curl session: " . curl_error($curl);}
					else {
						curl_close($curl);
						return $response;
					}
