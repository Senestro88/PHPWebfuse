...
class QrCodeImage extends \PHPWebFuse\Methods {
	...
	public function createBase64Image(string $accountName, string $secret, ?string $issuer = null): string {
		...
		if (!defined('QR_MODE_NUL')) {parent::loadLib("phpqrcode" . DIRECTORY_SEPARATOR . "qrlib");}
		...
		if (self::makeDir($tempPathname) && class_exists('\QRcode')) {
			$realPath = $tempPathname . '' . parent::randUnique("key") . '.png';
			\QRcode::png($content, $realPath, QR_ECLEVEL_Q, 20, 2);
			if (parent::isFile($realPath)) {
				clearstatcache(false, $realPath);
				$mime = mime_content_type($realPath);
				$baseEncode = base64_encode((string) parent::getFileContent($realPath));
				$data = 'data:' . $mime . ';base64,' . $baseEncode;
				parent::deleteFile($realPath);
				return $data;
			}
		}
		return "";
	...
	public function createOuputImage(string $accountName, string $secret, ?string $issuer = null): void {
		...
		if (self::makeDir($tempPathname) && class_exists('\QRcode')) {
			$realPath = $tempPathname . '' . parent::randUnique("key") . '.png';
			\QRcode::png($content, $realPath, QR_ECLEVEL_Q, 4, 2);
			if (parent::isFile($realPath)) {
