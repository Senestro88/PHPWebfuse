...
class Color {
    ...
    public static function getColorHexByName(string $colorName): string {
        // Normalize color names to lowercase for case-insensitive comparison.
        $colors = \array_change_key_case(self::colors(), \CASE_LOWER);
        // Iterate through the colors array to find the matching color name.
        foreach ($colors as $name => $arr) {
            if (trim(strtolower($colorName)) == trim($name)) {
                // Return the hex value if the color name matches.
                return $arr['hex'];
            }
        }
        // Return an empty string if no matching color name is found.
        return "";
    }
    ...
    public static function getColorNameByHex(string $colorHex): string {
        // Normalize hex values to lowercase for case-insensitive comparison.
        $colors = \array_change_key_case(self::colors(), \CASE_LOWER);
        // Iterate through the colors array to find the matching hex value.
        foreach ($colors as $name => $arr) {
            if (trim(strtolower($colorHex)) == trim(strtolower($arr['hex']))) {
                // Return the color name if the hex value matches.
                return $name;
            }
        }
        // Return an empty string if no matching hex value is found.
        return "";
    }

    /**
     * Check if a color hex value is present in the predefined colors list.
     *
     * @param string $colorHex The hex value of the color to check.
     * @return bool True if the color hex value is present; otherwise, false.
     */
    public static function isColorHexPresent(string $colorHex): bool {
        // Normalize hex values to lowercase for case-insensitive comparison.
        $colors = \array_change_key_case(self::colors(), \CASE_LOWER);
        // Iterate through the colors array to check for the presence of the hex value.
        foreach ($colors as $name => $arr) {
            if (trim(strtolower($colorHex)) == trim(strtolower($arr['hex']))) {
                // Return true if the color hex value is found.
                return true;
            }
        }
        // Return false if no matching hex value is found.
        return false;
    }

    /**
     * Check if a color name is present in the predefined colors list.
     *
     * @param string $colorName The name of the color to check.
     * @return bool True if the color name is present; otherwise, false.
     */
    public static function isColorNamePresent(string $colorName): bool {
        // Normalize color names to lowercase for case-insensitive comparison.
        $colors = \array_change_key_case(self::colors(), \CASE_LOWER);
        // Iterate through the colors array to check for the presence of the color name.
        foreach ($colors as $name => $arr) {
            if (trim(strtolower($colorName)) == trim($name)) {
                // Return true if the color name is found.
                return true;
            }
        }
        // Return false if no matching color name is found.
        return false;
    }
