...
class Database extends Utils {
    ...
    public function escape(string $string): string {
        return Utils::isNonNull($this->connection) ? $this->connection->real_escape_string($string) : addslashes($string);
    }
    ...
    public function createDatabase(string $name, string $character = "latin1", string $collate = " latin1_general_ci"): bool {
        return Utils::isNonNull($this->connection) ? $this->connection->query("CREATE DATABASE IF NOT EXISTS `" . strtolower($name) . "` DEFAULT CHARACTER SET " . $character . " COLLATE " . $collate . ";") : false;
    }
    ...
    public function deleteDatabaseTable(string $database, string $table): bool {
        if (Utils::isNonNull($this->connection)) {
            ...
            return Utils::isNotFalse($this->connection->query('DROP TABLE IF EXISTS ' . $database . '.' . $table . ';'));
        }
        return false;
    }
     ...
     *
     * This method facilitates inserting data into a specified database table.
     * It prepares and executes an SQL statement with the provided data.
     *
     * @param string $database The name of the database.
     * @param string $table The name of the database table.
     * @param array $data Associative array representing the data to insert (column => value).
     * @param bool $prepare Whether to prepare the statement or directly execute it.
     * @return bool|\mysqli_stmt Returns a mysqli_stmt object if prepared, or true on success, false on failure.
     */
    public function insertToDatabaseTable(string $database, string $table, array $data = [], bool $prepare = true): bool|\mysqli_stmt {
        // Check for valid database connection and non-empty data
        if (Utils::isNonNull($this->connection) && Utils::isNotEmptyArray($data)) {
            ...
            $values = array_map(fn($value) => Utils::isInt($value) ? $this->escape($value) : '"' . $this->escape((string) $value) . '"', $data);
    ...
    public function updateDatabaseTable(string $database, string $table, array $data = [], array $whereKeys = [], bool $prepare = true): bool|\mysqli_stmt {
        if (Utils::isNonNull($this->connection) && Utils::isNotEmptyArray($data) && Utils::isNotEmptyArray($whereKeys)) {
            ...
            $updateFields = array_map(fn($key, $value) => sprintf("`%s` = %s", $key, Utils::isString($value) ? '"' . $this->escape($value) . '"' : $this->escape($value)), array_keys($data), $data);
            // Prepare the WHERE clause
            $whereFields = array_map(fn($key, $value) => sprintf("`%s` = %s", $key, Utils::isString($value) ? '"' . $this->escape($value) . '"' : $this->escape($value)), array_keys($whereKeys), $whereKeys);
    ...
    public function createDatabaseTable(string $database, string $table, array $columns = [], string $comment = '', string $engine = "MyISAM", string $character = "latin1", string $collate = "latin1_general_ci", bool $autoIncrement = true): bool {
        if (Utils::isNonNull($this->connection)) {
            ...
            // Build the CREATE TABLE statement
            $columnsDefinition = array_map(fn($name, $definition) => "$name $definition", array_keys($columns), $columns);
            $autoIncrementOption = $autoIncrement ? " AUTO_INCREMENT=1" : "";
            $statement = sprintf("CREATE TABLE IF NOT EXISTS %s.%s (%s) ENGINE=%s DEFAULT CHARSET=%s COLLATE=%s%s", $database, $table, implode(", ", $columnsDefinition), $engine, $character, $collate, $autoIncrementOption);
                    ...
                    while ($table = $tables->fetch_array()) {
                        ...
                        if ($columns instanceof \mysqli_result) {
                            ...
                            foreach ($columnsData as $data) {
                                ...
                                // Build alteration statements for specific field types
                                if ($field == "id" && $key == "PRI") {
    ...
    public function getTableRowsWhereClause(string $database, string $table, string $column, mixed $columnValue, string $orderBy = "id"): array {
        ...
        if (Utils::isNonNull($this->connection)) {
            ...
            $result = $this->getAssociationFromSelectStatement($statement);
        }
        return $result;
    }
     ...
     * @return array
     */
    public function getAssociationFromSelectStatement(string $statement): array {
        $result = array();
        if (Utils::isNonNull($this->connection) && Utils::startsWith("SELECT", strtoupper($statement))) {
