...
class EThumbnail {
	 ...
	 * Auto scale landscape or portrait
	 *
	 * @var int
	 */
	public $Thumbsize;
	...
	private function thumbmaker() {
		...
		if ($this->loadimage()) {
			...
			if ($this->Cropimage[0] > 0) {
				$this->cropimage();
			}
			if ($this->Addtext[0] > 0) {
				$this->addtext();
			}
			if ($this->Medianfilter) {
				$this->medianfilter();
			}
			if ($this->Greyscale) {
				$this->greyscale();
			...
			if ($this->Colorize[0] == 1) {
				$this->colorize();
			}
			if ($this->Colorreplace[0] == 1) {
				$this->colorreplace();
			}
			if ($this->Pixelscramble[0] == 1) {
				$this->pixelscramble();
			}
			if ($this->Pixelate[0] == 1) {
				$this->pixelate();
			}
			if ($this->Ageimage[0] == 1) {
				$this->ageimage();
			}
			if ($this->Fliphorizontal) {
				$this->rotateorflip(0, 1);
			}
			if ($this->Flipvertical) {
				$this->rotateorflip(0, -1);
			}
			if ($this->Watermarkpng != '') {
				$this->addpngwatermark();
			}
			if ($this->Clipcorner[0] == 1) {
				$this->clipcornersstraight();
			}
			if ($this->Clipcorner[0] == 2) {
				$this->clipcornersround();
			}
			...
			if ($this->Ripplefx[0] == 1) {
				$this->ripplefx();
			}
			if ($this->Lakefx[0] == 1) {
				$this->lakefx();
			}
			if ($this->Waterdropfx[0] == 1) {
				$this->waterdropfx();
			}
			if ($this->Displacementmap[0] == 1) {
				$this->displace();
			}
			if ($this->Negative) {
				$this->negative();
			...
			if ($this->Displacementmapthumb[0] == 1) {
				$this->displacethumb();
			}
			if ($this->Perspectivethumb[0] == 1) {
				$this->perspectivethumb();
			}
			if ($this->Maketransparent[0] == 1) {
			...
			switch ($this->size[2]) {
				case 1:
					if (imagetypes() & IMG_GIF) {
						$this->im = imagecreatefromgif($this->image);
						return true;
					} else {
						$this->invalidimage('No GIF support');
						return false;
					}
					break;
				case 2:
					if (imagetypes() & IMG_JPG) {
						...
						$this->invalidimage('No JPG support');
						return false;
					}
					break;
				case 3:
					if (imagetypes() & IMG_PNG) {
						...
						$this->invalidimage('No PNG support');
						return false;
					}
					break;
				default:
	...
	private function createemptythumbnail() {
		...
		if ($this->Percentage) {
			if ($thumbwidth > 0) {
				$thumbwidth = floor(($thumbwidth / 100) * $this->size[0]);
			}
			if ($thumbheight > 0) {
				$thumbheight = floor(($thumbheight / 100) * $this->size[1]);
			}
		...
		if (intval($clipsize) > 0) {
			...
			for ($i = 0; $i < $clipsize; $i++) {
				if ($this->Clipcorner[3] && $random1) {
					imageline($this->im, 0, $i, $clipsize - $i, $i, $bgcolor);
				}
				if ($this->Clipcorner[4] && $random2) {
					imageline($this->im, 0, $this->size[1] - $i - 1, $clipsize - $i, $this->size[1] - $i - 1, $bgcolor);
				}
				if ($this->Clipcorner[5] && $random3) {
					imageline($this->im, $this->size[0] - $clipsize + $i, $i, $this->size[0] + $clipsize - $i, $i, $bgcolor);
				}
				if ($this->Clipcorner[6] && $random4) {
					imageline($this->im, $this->size[0] - $clipsize + $i, $this->size[1] - $i - 1, $this->size[0] + $clipsize - $i, $this->size[1] - $i - 1, $bgcolor);
				}
			}
		}
	}
	...
	private function addbinder() {
		...
		if (intval($this->Binderspacing) < 4) {
			$this->Binderspacing = 4;
		}
	...
	private function rotateorflip($rotate, $flip) {
		...
		if (intval($this->Rotate) > 0 || $flip > 0) {
			for ($px = 0; $px < $this->size[0]; $px++) {
				if ($rotate) {
					for ($py = 0; $py < $this->size[1]; $py++) {
						imagecopy($this->newimage, $this->im, $this->size[1] - $py - 1, $px, $px, $py, 1, 1);
					}
				} else {
					for ($py = 0; $py < $this->size[1]; $py++) {
						imagecopy($this->newimage, $this->im, $this->size[0] - $px - 1, $py, $px, $py, 1, 1);
					}
				}
			}
		} else {
			for ($px = 0; $px < $this->size[0]; $px++) {
				if ($rotate) {
					for ($py = 0; $py < $this->size[1]; $py++) {
						imagecopy($this->newimage, $this->im, $py, $this->size[0] - $px - 1, $px, $py, 1, 1);
					}
				} else {
					for ($py = 0; $py < $this->size[1]; $py++) {
						imagecopy($this->newimage, $this->im, $px, $this->size[1] - $py - 1, $px, $py, 1, 1);
					}
				}
			}
		}
	...
	private function medianfilter() {
		...
		for ($y = 0; $y < $this->size[1]; $y++) {
			for ($x = 0; $x < $this->size[0]; $x++) {
				...
				$newgreen[] = $a33['green'];
				$newblue[] = $a33['blue'];
				sort($newred, SORT_NUMERIC);
				sort($newgreen, SORT_NUMERIC);
				sort($newblue, SORT_NUMERIC);
	...
	private function bilinear($xnew, $ynew) {
		...
		$fracy = $ynew - $yf;
		$fracy1 = 1 - $fracy;
		$ff = $this->rgbpixel($xf, $yf);
		$cf = $this->rgbpixel($xc, $yf);
		$fc = $this->rgbpixel($xf, $yc);
		$cc = $this->rgbpixel($xc, $yc);
	...
	private function waterdropfx() {
		...
		for ($y = 0; $y < $this->size[1]; $y++) {
			for ($x = 0; $x < $this->size[0]; $x++) {
				...
				if ($distance != 0) {
					$amount = $amount * $this->Waterdropfx[3] / $distance;
				}
				$newx = $x + $truex * $amount;
				$newy = $y + $truey * $amount;
				$newpix = $this->bilinear($newx, $newy);
	...
	private function maketransparent() {
		...
		$red = hexdec(substr($this->Maketransparent[2], 1, 2));
		$green = hexdec(substr($this->Maketransparent[2], 3, 2));
		$blue = hexdec(substr($this->Maketransparent[2], 5, 2));
	...
	private function displace() {
		...
		if (file_exists($this->Displacementmap[1])) {
			...
			switch ($size[2]) {
				case 1:
					if (imagetypes() & IMG_GIF) {
						$map = imagecreatefromgif($this->Displacementmap[1]);
					} else {
				...
				case 2:
					if (imagetypes() & IMG_JPG) {
						$map = imagecreatefromjpeg($this->Displacementmap[1]);
					} else {
						$map = imagecreatetruecolor(100, 100);
					}
					break;
				case 3:
					if (imagetypes() & IMG_PNG) {
						$map = imagecreatefrompng($this->Displacementmap[1]);
					} else {
						$map = imagecreatetruecolor(100, 100);
					}
					break;
